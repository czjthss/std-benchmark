from RobustSTL.utils import *
from cvxopt import matrix
from RobustSTL.l1 import l1
...
def seasonality_extraction(sample, season_len=10, K=2, H=5, ds1=50., ds2=1.):
    ...
    def get_season_value(idx):
        ...
        season_value = np.sum(weight_sample * weights) / np.sum(weights) if np.abs(np.sum(weights)) > 0.1 else np.sum(
            weight_sample * weights)
        return season_value
